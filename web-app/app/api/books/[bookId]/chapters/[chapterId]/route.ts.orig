...
export async function GET(
  ...
  try {
    const chapters = loadChapters(params.bookId);
    const chapter = chapters.find((ch) => ch.chapter_id === params.chapterId);
...
export async function PUT(
  ...
  try {
    ...
    const book = index.books.find(b => b.id === params.bookId);
    ...
    if (!fs.existsSync(path.join(harnessRoot, projectYaml))) {
      projectYaml = `projects/${params.bookId}.yaml`;
      if (!fs.existsSync(path.join(harnessRoot, projectYaml))) {
    ...
    try {
      const { stdout, stderr } = await execAsync(`python3 -m pyharness write --project "${projectYaml}" --chapter "${params.chapterId}"`, {
        cwd: harnessRoot,
      });
...
export async function DELETE(
  ...
  try {
    ...
    const harnessRoot = path.resolve(process.cwd(), "..");
    const chapterPath = path.join(harnessRoot, "knowledge", book.name, "chapters", `${params.chapterId}.json`);
